...
signal fsm_bound : STD_LOGIC_VECTOR (27 downto 0) := x"00000F0";
...
fake_clk <= not fake_clk after  1 ns;

BTNC <= '1' after 4030ms;

disp_clk_divider : clock_divider port map (CLK => fake_clk, UPPERBOUND => disp_bound, SLOWCLK => disp_clk);
fsm_clk_divider : clock_divider port map (CLK => fake_clk, UPPERBOUND => fsm_bound, SLOWCLK => fsm_clk);
